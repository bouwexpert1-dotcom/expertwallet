...
export default function PayPalDepositModal({ onConfirm, onClose }) {
  ...
  function handleAmountSubmit(e) {
    ...
    if (!amt || amt < 1) { setError(t('minimumDeposit')); return; }
    if (!email) { setError(t('enterPaypalEmail')); return; }
    setError("");
    setStep("processing");
      ...
      <div className="bg-wallet-card border border-wallet-border rounded-3xl w-full max-w-md p-6 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-white text-xl font-bold">{t('paypalDeposit')}</h2>
          </div>
          {step !== "processing" && (
        ...
        {step === "amount" && (
          <form onSubmit={handleAmountSubmit} className="flex flex-col gap-4">
            <div>
              <label className="text-wallet-muted text-xs uppercase tracking-widest mb-2 block">{t('selectAmount')}</label>
            ...
            <div>
              <label className="text-wallet-muted text-xs uppercase tracking-widest mb-1 block">{t('paypalEmail')}</label>
              <input
                type="email"
        ...
        {step === "processing" && (
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <p className="text-white font-semibold text-lg">{t('processingPayment')}</p>
            <p className="text-wallet-muted text-sm">{t('connectingPaypal')}</p>
        ...
        {step === "success" && (
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-white font-bold text-xl">{t('paymentSuccessful')}</p>
            <p className="text-wallet-muted text-sm">
              <span className="text-wallet-gold font-semibold">${parseFloat(amount).toFixed(2)}</span> {t('willBeAdded')}
            </p>
            <div className="w-full bg-green-400/10 border border-green-400/30 rounded-xl px-4 py-3 text-xs text-green-300 text-center">
              {t('transactionID')} PP-{Date.now().toString().slice(-10).toUpperCase()}
            </div>
            <button
        ...
        {step === "failed" && (
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-white font-bold text-xl">{t('paymentFailed')}</p>
            <p className="text-wallet-muted text-sm text-center">{t('couldNotProcess')}</p>
